...
The system uses Natural Language Processing (NLP) and a trained Logistic Regression model to analyze the text and classify it as Real or Fake along with a confidence score. Additionally, the system integrates NewsAPI to verify news from trusted real-time sources.
...
- Verifies news using NewsAPI for real-time validation  
...
8. Backend first checks news using NewsAPI  
9. If similar news is found → classify as Real  
10. If not found → ML model predicts result  
11. Display prediction (Real/Fake + confidence)  
12. Store prediction history in PostgreSQL  
13. Show viral fake news from JSON  

---

## 🌐 NewsAPI Integration

To improve accuracy, TruthLens integrates NewsAPI for real-time verification.

- The system searches the input news in trusted sources  
- If found → classified as Real  
- If not found → ML model is used  
